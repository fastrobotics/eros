...
std::string unittest_nodename = "{{cookiecutter.node_name_binary}}";
...
TEST({{cookiecutter.node_classname}}, TestBasics) {
    ASSERT_TRUE(false);
    ros::NodeHandle nh("~");
    Logger* logger = new Logger("DEBUG", "test_{{cookiecutter.node_classname}}");
    ...
    sleep(1.0);  // Wait for {{cookiecutter.node_classname}} to Start.
...
int main(int argc, char** argv) {
    ...
    ros::init(argc, argv, "test_{{cookiecutter.node_classname}}");
